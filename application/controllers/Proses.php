...
class Proses extends CI_Controller {
    ...
    function tambah_siswa() {
        ...
        if(@$pass) {
            $password = password_hash($pass, PASSWORD_BCRYPT, ['cost' => 11]);
        } else {
            $password = password_hash('12345', PASSWORD_BCRYPT, ['cost' => 11]);
        }

        // insert to tb_akun
        $this->db->insert('tb_akun', [
    ...
    function edit_siswa() {
        $siswa = $this->db->where('id_siswa', $this->input->post('id_siswa'))->get('tb_siswa')->row();
        $pass_baru = $this->input->post('password');

        if(@$pass_baru) {
            $pass = password_hash($pass_baru, PASSWORD_BCRYPT, ['cost' => 11]);
        } else {
            $pass = password_hash($this->input->post('old_password'), PASSWORD_BCRYPT, ['cost' => 11]);
        }
    ...
    public function hapus_siswa() {
        ...
        // delete absen siswa
        $this->m_admin->hapus('tb_absiswa', 'siswa', $id);

        // delete ekskul
        $this->m_admin->hapus('tb_ekskul', 'siswa', $id);
            ...
            $this->session->set_flashdata('siswa',
                "<div class='uk-alert uk-alert-danger' data-uk-alert>
                    <a href='#' class='uk-alert-close uk-close'></a>
                            ...
                            Harap pilih siswa mana yang ingin dihapus
                            </div>");    
        } else {
    ...
    function get_kesehatan($id, $level) {
                        ...
                        <div class="md-card-content">
                            <div class="uk-grid">
                                ...
                                <div class="uk-width-3-10 uk-text-right">';
                                    if($level != 3) {
                                        echo '<button data-uk-modal="{target:' . $pt . '#tambah_ekskul' . $pt . '}" onclick="tambah_ekskul(' . $kl->id_tahunajaran . ')" class="md-btn md-btn-small md-btn-primary md-btn-wave-light"><i class="fas fa-plus-circle"></i></button>';
                                    }
                                echo '    
                                </div>
                            </div>
    ...
    function get_prestasi($id, $level) {
                    ...
                    <div class="md-card">
                        <div class="md-card-content">
                            <div class="uk-grid">
                                ...
                                <div class="uk-width-3-10 uk-text-right">';
                                if($level != 3) {
                                    echo '<button data-uk-modal="{target:' . $pt . '#tambah_prestasi' . $pt . '}" onclick="tambah_prestasi(' . $kl->id_tahunajaran . ')" class="md-btn md-btn-small md-btn-primary md-btn-wave-light"><i class="fas fa-plus-circle"></i></button>';
                                }
                                echo '    
                                </div>
                            </div>
                ...
                if ($cekdata > 0) {
                    foreach ($absiswa->result() as $ag) {
                        if ($ag->siswa == $id && $ag->ta == $id_tj && $ag->smt == $id_smt && $ag->ab == $ab->id_ab) {
                            ...
                            if($level != 3) {
                                echo '<td><a data-uk-tooltip="{pos:top}" title="Hapus" href="#" class="md-btn md-btn-small md-btn-danger md-btn-wave-light" onclick="hapus_absiswa(' . $ag->id_absiswa . ')"><i class="feather icon-trash"></i></a>
...
<td>3.' . $x++ . '</td>
            ...
            <td><input class="md-input na_kd-' . ($x - 1) . '-' . $sw->id_nilai_ki3 . '" name="na_kd[]" value="' . $k3->na_kd . '" id="na_kd-' . $k3->id_nki3 . '-' . $sw->id_nilai_ki3 . '" autocomplete="off" readonly=""></td>
                ...
                echo '<input type="hidden" name="total_na[]" id="total_na-' . $sw->id_nilai_ki3 . '" value="' . number_format($avg[0]->avg) . '">';
...
<td>3.' . $x++ . '</td>
    ...
    function get_nilai_ki4($kl, $id, $ta) {
        $tj    = $this->m_admin->data_tahunajaran()->result();
        $siswa = $this->db->query("SELECT * FROM tb_siswa WHERE kelas=" . $kl . " ");
        $n     = $this->db->query("SELECT a.*, b.* FROM tb_nilai_ki4 a, tb_siswa b WHERE a.siswa = b.id_siswa AND a.mapel=" . $id . " AND a.kelas=".$kl." ");
        $smt   = 0;
        foreach ($tj as $t) {
            if ($t->id_tahunajaran == $ta) {
                $smt = $t->smt;
...
<td>3.' . $x++ . '</td>
            ...
            <td><input class="md-input na_kd-' . ($x - 1) . '-' . $sw->id_nilai_ki4 . '" name="na_kd[]" value="' . $k3->na_kd . '" id="na_kd-' . $k3->id_nki4 . '-' . $sw->id_nilai_ki4 . '" autocomplete="off" readonly=""></td>
                ...
                echo '<input type="hidden" name="total_na[]" id="total_na-' . $sw->id_nilai_ki4 . '" value="' . number_format($avg[0]->avg) . '">';
