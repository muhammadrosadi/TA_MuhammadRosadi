...
class Admin extends CI_Controller {
    ...
    function tambah_guru() {
        $this->db->insert('tb_akun', [
            'username' => $this->input->post('nip') . '-' . rand(111, 333),
...
// DESKRIPSI KOMPETENSI
    ...
    function detailsiswa($id) {
        ...
        $var['kelas']       = $this->m_admin->data_kelas();
        $var['siswa']       = $this->m_admin->data_siswa();
        // $var['sw']          = $this->m_admin->get_siswa($id)->row_array();
        $var['mapel']       = $this->db->where('kode_mapel != 1')->get('tb_mapel');
    ...
    function rekap_nilai_ki3($kl, $id, $ta) {
        ...
        $var['ki3']    = $this->db->query("SELECT * FROM tb_desk_ki3 WHERE mapel=" . $id . " AND kelas=".$kl." ")->result();
        $var['active'] = 'siswa';
        $var['kl']     = $kl;
        $var['id']     = $id;
        $var['ta']     = $ta;
